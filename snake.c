...
void snake_draw_board()
{
    ...
    {
        //游戏面板边界显示为 XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
        snake_write_text(i,0,        "X");
        snake_write_text(i,g_width,  "X");
    }
    
    for(i=1;i<g_width;i++)
    {
        snake_write_text(0,i,        "X");
        snake_write_text(g_height,i, "X");
    }
...
int snake_coordinate_to_index(pos position)
{
...
bool snake_move_player(pos head)
{
    ...
    //将位置坐标转换成位置数组索引
    int idx = snake_coordinate_to_index(head);
    ...
    else
    {
        ...
        // 将该位置置为空闲
        spaces[snake_coordinate_to_index( *tail )] = false;
